...
define(['data/DashboardService'], function(DashboardService) {
  ...
  TestList = cell.extend((function() {
    var displayOrder, statusDisplayable;
    ...
    return {
      render: function(R) {
        return DashboardService.getStoryTestDetails(this.model.storynum, __bind(function(_arg) {
    ...
    bind: {
      load: function() {
        if (!this.$('#testList').length) {
          return $(this.el).append(new TestList({
            id: 'testList',
            model: this.model
          }).el);
        }
