define ['data/DashboardService'],(DashboardService)->
  ...
  TestList = cell.extend do->
    ...
    render: (R)->
      DashboardService.getStoryTestDetails @model.storynum, ({tests})=>
        @model.tests = tests
        numVisibleGroups = 0
        oddEven = do->
          isEven = false
  ...
  bind:
    load: ->
      # Lazy load/render
      if not @$('#testList').length
        $(@el).append new TestList(id:'testList', model:@model).el
